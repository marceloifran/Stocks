...
<body>
    <div class="container">
        <h1 class="h1 text-center alert alert-info">Escaneo</h1>
        <div style="margin: 5px" class="text-center" data-toggle="buttons">
            <label class="btn btn-primary active">
              <input type="radio" name="options" value="1" autocomplete="off" checked> Frente
            </label>
            <label class="btn btn-secondary">
              <input type="radio" name="options" value="2" autocomplete="off"> Detras
            </label>
          </div>

        <div class="text-center">
            <video style="width: 70%" id="preview" class="text-center"></video>
        </div>
        ...
        <div style="padding: 10px" class="col-lg-4 mx-auto">
            <div style="display: none">
                <h2 class="fs-4">Personas Escaneadas</h2>
                <ul id="listaAsistencia" class="fs-5"></ul>
            </div>

            <h1 class="fs-4 alert alert-info text-center">Personas Validadas: <span
                    id="contadorPersonas">0</span></h1>

            <h3 id="validadosList" class="text-center"></h3>

            <div class="form-group text-center" style="padding: 10px">
                <label for="tipoAsistencia" class="fs-4 alert alert-primary text-center">Tipo de Asistencia
                    </label>
                <select id="tipoAsistencia" class="form-control fs-5">
                    <option value="entrada">Entrada </option>
                    <option value="salida">Salida</option>
                </select>
                <img src="{{ asset('/images/logo.png') }}" style="width: 50%" alt="" class="imagen-personal">
            </div>
        </div>
    </div>
</body>
...
<script type="text/javascript">
    ...
    // });
    //  let scanner2 = new Instascan.Scanner({ video: document.getElementById('preview') , mirror:false});
    ...
    setInterval(finalizarAsistenciaPeriodica, 1 * 60 * 1000);

    function finalizarAsistenciaPeriodica() {
        // Verificar si hay códigos de asistencia para guardar
        if (asistenciaData.length > 0) {
            console.log('Guardando asistencia periódicamente...');

            // Simular una solicitud al servidor para guardar los datos
            // Reemplaza esto con tu lógica real de envío al servidor
            axios.post('/guardar-asis', {
                ...
                .then(function(response) {
                    ...
                    asistenciaData.length = 0; // Limpiar el array de datos de asistencia
